...
func submit_task_array_parameterized(
	instance: Object, method: String, parameter: Array, task_tag = null
) -> Future:
...
func __enqueue_task(
	...
	var result = Future.new(
		instance, method, parameter, task_tag, no_argument, array_argument, self
	)
	if __finished:
		result.__finish()
		return result
